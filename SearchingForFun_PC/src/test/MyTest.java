...
public class MyTest {
	...
	public void testBFSPuzzle() {
		assertEquals(
				EightPuzzle.orderedEightPuzzle().toString(),
				sm.doSearch(EightPuzzle.randomEightPuzzle(),
						EightPuzzle.orderedEightPuzzle(), sf, agendaBFS).get(0)
						.getState().toString());
	}

	public void testDFSPuzzle() {
		// assertEquals(EightPuzzle.orderedEightPuzzle().toString() ,
		// sm.doSearch(EightPuzzle.randomEightPuzzle(),EightPuzzle.orderedEightPuzzle(),
		// sf, agendaDFS).get(0).getState().toString());
	}
	...
	public void testAStarPuzzle() {
		assertEquals(
				EightPuzzle.orderedEightPuzzle().toString(),
				starPuzzle
						.doSearch(EightPuzzle.randomEightPuzzle(),
								EightPuzzle.orderedEightPuzzle(), sf, agendaBFS)
						.get(0).getState().toString());
	}

	public void testBFSJumble() {
		assertEquals(
				"hello",
				jumble.doSearch(new Jumble("ollhe"), new Jumble("hello"), sfJ,
						agendaJBFS).get(0).getState().toString());
	}

	public void testDFSJumble() {
		assertEquals(
				"magic",
				jumble.doSearch(new Jumble("gmica"), new Jumble("magic"), sfJ,
						agendaJDFS).get(0).getState().toString());
	}

	public void testAStarJumble() {
		assertEquals(
				"friend",
				starJ.doSearch(new Jumble("idefnr"), new Jumble("friend"), sfJ,
						agendaJDFS).get(0).getState().toString());
	}
